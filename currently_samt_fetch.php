...
if (isset($_POST['reasons'])) {
    ...
    try {
        ...
        if ($reasons == 'sale') {
            $stmt = $pdo->prepare("SELECT o.*, p.*, u.u_username
                                   FROM stockout o
            ...
            $stmt = $pdo->prepare("SELECT o.*, p.*, u.u_username
                                   ...
                                   LEFT JOIN product p ON o.o_product_id = p.p_product_id
                                   LEFT JOIN user u ON u.u_userid = o.o_username
                                   WHERE o.o_return IS NULL AND o_pr_code IS NULL AND
                                   (o.o_store = '1' AND (o.o_reasons = 'sale sample' OR o.o_reasons = 'sale'))
                                   ORDER BY o_reasons, o.o_mg_code DESC");
        }
        ...
        foreach ($products as $index => $product) {
         ...
         ($data_reasons[1] == 2 ? 'SAKABA' : 'SAMT')) .
            ...
            echo "<td class='text-center'><span class='badge " . ($product['o_pr_code'] !== null ? 'badge-success' : 'badge-secondary') . "'>" . ($product['o_pr_code'] !== null ? 'issued' : '-') . "</span></td>";
